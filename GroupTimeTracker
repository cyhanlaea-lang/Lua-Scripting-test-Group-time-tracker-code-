-- this is complex system script for checking time of specific group rank spends in game and also sends reports in discord channel for logging weekly.
local DataStoreService = game:GetService("DataStoreService")
local Players = game:GetService("Players")
local HttpService = game:GetService("HttpService")
local RunService = game:GetService("RunService")

local GROUP_ID = 269811162
local GUARD_RANK = 11
...
local RETRY_DELAY = 5
local MAX_REQUESTS_PER_MINUTE = 30
local CACHE_DURATION = 90
	...
	while retries < MAX_DATASTORE_RETRIES do
		...
		if success then
			...
			if retries < MAX_DATASTORE_RETRIES then
				wait(RETRY_DELAY * math.pow(2, retries - 1))
			end
		end
	end
...
local function calculateWeekBoundaries()
	local now = os.time()
	local currentDay = tonumber(os.date("%w", now))
...
local function saveWeeklyData()
	...
	if currentTime - lastSaveTime < SAVE_COOLDOWN and not pendingSave then
		pendingSave = true
		delay(SAVE_COOLDOWN - (currentTime - lastSaveTime), function()
			pendingSave = false
			saveWeeklyData()
		end)
		return false
