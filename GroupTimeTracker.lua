...
Staff can use command !access to check activity (time) here i made min rank 0 so you can test it for application freely--]]
...
local function processRequestQueue()
    ...
    while #requestQueue > 0 do
        ...
        if requestCount >= MAX_REQUESTS_PER_MINUTE then
            ...
            print(string.format("DataStore rate limit reached. Queueing %d requests, waiting %d seconds...", #requestQueue + 1, waitTime))
        ...
        if request.callback then request.callback(success, result) end
        task.wait(0.1)
    end
...
local function queueDataStoreOperation(operation, callback, ...)
    table.insert(requestQueue, { operation = operation, args = {...}, callback = callback })
    if not processingQueue then task.spawn(processRequestQueue) end
end
...
local function setCachedSession(userId, data)
    sessionCache[userId] = { data = data, timestamp = os.time() }
end
    ...
    while retries < MAX_DATASTORE_RETRIES do
        ...
        if success then
            ...
            if not string.find(tostring(result), "RequestLimitReached") then
                print(string.format("DataStore operation failed (attempt %d/%d): %s", retries, MAX_DATASTORE_RETRIES, tostring(result)))
            end
            -- backoff
            if retries < MAX_DATASTORE_RETRIES then task.wait(RETRY_DELAY * math.pow(2, retries - 1)) end
        end
    end
...
local function hasGuardRank(userId)
    ...
    if cached and cached.rankCheck then return cached.rankCheck >= GUARD_RANK end
    local player = Players:GetPlayerByUserId(userId)
    if not player then return false end
    -- Get player's rank in group
    local success, result = pcall(function() return player:GetRankInGroup(GROUP_ID) end)
    if success then
        ...
        -- Handle API errors
        if not string.find(tostring(result), "429") then print("Error checking rank for user " .. userId .. ": " .. tostring(result)) end
        return false
    end
end
...
local function getPlayerRankName(userId)
    ...
    local success, result = pcall(function() return player:GetRoleInGroup(GROUP_ID) end)
...
local function getCurrentPlayerTime(userId)
    if not weeklyData.playerTimes or not weeklyData.playerTimes[userId] then return 0 end
    local totalTime = weeklyData.playerTimes[userId].time or 0
    -- we add active session time if player is online
    if activeSessions[userId] then totalTime = totalTime + (os.time() - activeSessions[userId]) end
    return totalTime
end
...
local function saveWeeklyData()
    if not initialLoadComplete then return false end
    local currentTime = os.time()
    ...
    local success, err = safeDataStoreOperation(function()
        ...
        task.spawn(function()
            local backupSuccess = pcall(function() backupStore:SetAsync("backup_" .. os.date("%Y%m%d"), saveData) end)
            if backupSuccess then print("Backup saved successfully") end
        end)
    end)
    if success then
        lastSaveTime = os.time()
        local playerCount = 0
        for _ in pairs(saveData.playerTimes) do playerCount = playerCount + 1 end
        print(string.format("Weekly data saved successfully at %s. Players: %d", os.date("%H:%M:%S"), playerCount))
        return true
    else
        ...
        if string.find(tostring(err), "RequestLimitReached") then
            ...
            task.spawn(function()
                ...
                local emergencySuccess = pcall(function()
                    ...
                    emergencyStore:SetAsync("emergency_save", { timestamp = os.time(), playerCount = #Players:GetPlayers() })
                end)
                if emergencySuccess then print("Emergency save completed") end
            end)
        end
        return false
    end
...
local function sendWeeklyReportToDiscord()
    ...
    for userId, data in pairs(weeklyData.playerTimes) do
        if data and data.name and data.time then
            table.insert(sortedPlayers, { name = data.name, time = data.time, rank = data.rank or "Unknown" })
    ...
    local summaryEmbed = {
        ...
        color = 3447003,
        fields = {},
        footer = { text = "Automated Report • " .. os.date("%Y-%m-%d %H:%M:%S") }
    }
    ...
    table.insert(summaryEmbed.fields, { name = "📅 Period", value = startTimeText .. " to " .. endTimeText, inline = false })
    -- Calculate totals
    local totalTime = 0
    for _, player in pairs(sortedPlayers) do totalTime = totalTime + (player.time or 0) end
    local averageTime = playerCount > 0 and (totalTime / playerCount) or 0
    table.insert(summaryEmbed.fields, {
        name = "Summary",
        value = string.format("**Total Players:** %d\n**Total Time:** %s\n**Average Time:** %s", playerCount, formatTime(totalTime), formatTime(averageTime)),
        inline = false
    })
    ...
    if playerCount > 0 then
        ...
        for i = 1, playerCount, 20 do
            local chunk = {}
            for j = i, math.min(i + 19, playerCount) do table.insert(chunk, sortedPlayers[j]) end
        ...
        for chunkIndex, chunk in ipairs(chunks) do
            local embed = {
                ...
                color = 15105570,
                fields = {},
                footer = { text = string.format("Page %d/%d • Total Players: %d", chunkIndex, #chunks, playerCount) }
            }
            local playersText = ""
            for i, player in ipairs(chunk) do
                ...
                playersText = playersText .. string.format("%s%s (%s) - `%s`\n", medal, player.name or "Unknown", player.rank or "Unknown", formatTime(player.time or 0))
            end
        ...
        local noDataEmbed = {
            ...
            footer = { text = "No Data • " .. os.date("%Y-%m-%d %H:%M:%S") }
        }
    ...
    -- Send to Discord
    local success, result = pcall(function() HttpService:PostAsync(DISCORD_WEBHOOK_URL, HttpService:JSONEncode(payload)) end)
    if success then
...
local function loadWeeklyData()
    ...
    local function loadFromStore(store, isBackup)
        local success, data = safeDataStoreOperation(function() return store:GetAsync(isBackup and "backup_" .. os.date("%Y%m%d") or "weeklyData") end)
            ...
            if data.playerTimes then
                ...
                for userIdStr, playerData in pairs(data.playerTimes) do
                    ...
                    if userId and playerData then
                        convertedPlayerTimes[userId] = { name = playerData.name or "Unknown", time = playerData.time or 0, rank = playerData.rank or "Unknown" }
                    end
                end
                data.playerTimes = convertedPlayerTimes
            end
            if data.startTime and data.endTime and data.playerTimes then return data end
        end
        return nil
    end
    ...
    if data then
        ...
        weeklyData.startTime, weeklyData.endTime = calculateWeekBoundaries()
        weeklyData.playerTimes = {}
        if not RunService:IsStudio() then task.spawn(function() task.wait(60); saveWeeklyData() end) end
    end
...
local function showTimeRecords(player)
    if player.PlayerGui:FindFirstChild("TimeTrackerGUI") then player.PlayerGui.TimeTrackerGUI:Destroy() end
    local gui = Instance.new("ScreenGui")
    gui.Name = "TimeTrackerGUI"; gui.Parent = player.PlayerGui; gui.ResetOnSpawn = false; gui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    local mainFrame = Instance.new("Frame")
    mainFrame.Name = "MainFrame"; mainFrame.Parent = gui
    mainFrame.Size = UDim2.new(0.7, 0, 0.65, 0); mainFrame.Position = UDim2.new(0.5, 0, 0.5, 0)
    mainFrame.AnchorPoint = Vector2.new(0.5, 0.5); mainFrame.BackgroundColor3 = Color3.fromRGB(40, 40, 55)
    mainFrame.BorderSizePixel = 0; mainFrame.ClipsDescendants = true
    local aspectRatioConstraint = Instance.new("UIAspectRatioConstraint")
    aspectRatioConstraint.AspectRatio = 1.5; aspectRatioConstraint.Parent = mainFrame
    local sizeConstraint = Instance.new("UISizeConstraint")
    sizeConstraint.MinSize = Vector2.new(300, 280); sizeConstraint.MaxSize = Vector2.new(500, 450); sizeConstraint.Parent = mainFrame
    local corner = Instance.new("UICorner"); corner.CornerRadius = UDim.new(0, 8); corner.Parent = mainFrame
    local header = Instance.new("Frame")
    header.Name = "Header"; header.Parent = mainFrame; header.Size = UDim2.new(1, 0, 0.2, 0)
    header.Position = UDim2.new(0, 0, 0, 0); header.BackgroundColor3 = Color3.fromRGB(50, 50, 70); header.BorderSizePixel = 0
    local headerCorner = Instance.new("UICorner"); headerCorner.CornerRadius = UDim.new(0, 8); headerCorner.Parent = header
    local title = Instance.new("TextLabel")
    title.Name = "Title"; title.Parent = header; title.Size = UDim2.new(0.8, 0, 0.3, 0)
    title.Position = UDim2.new(0.1, 0, 0.1, 0); title.BackgroundTransparency = 1
    title.Text = "WEEKLY TIME TRACKER"; title.TextColor3 = Color3.fromRGB(255, 255, 255)
    title.TextXAlignment = Enum.TextXAlignment.Left; title.Font = Enum.Font.GothamBold; title.TextScaled = true
    local period = Instance.new("TextLabel")
    period.Name = "Period"; period.Parent = header; period.Size = UDim2.new(0.8, 0, 0.2, 0)
    period.Position = UDim2.new(0.1, 0, 0.4, 0); period.BackgroundTransparency = 1
    period.Text = "Period: " .. formatDate(weeklyData.startTime) .. " to " .. formatDate(weeklyData.endTime)
    period.TextColor3 = Color3.fromRGB(200, 200, 200); period.TextXAlignment = Enum.TextXAlignment.Left
    period.Font = Enum.Font.Gotham; period.TextSize = 12; period.TextWrapped = true
    local countdownLabel = Instance.new("TextLabel")
    countdownLabel.Name = "CountdownLabel"; countdownLabel.Parent = header; countdownLabel.Size = UDim2.new(0.8, 0, 0.2, 0)
    countdownLabel.Position = UDim2.new(0.1, 0, 0.65, 0); countdownLabel.BackgroundTransparency = 1
    countdownLabel.Text = "Next reset: Calculating..."; countdownLabel.TextColor3 = Color3.fromRGB(100, 220, 100)
    countdownLabel.TextXAlignment = Enum.TextXAlignment.Left; countdownLabel.Font = Enum.Font.GothamBold
    countdownLabel.TextSize = 12; countdownLabel.TextWrapped = true
    local closeButton = Instance.new("TextButton")
    closeButton.Name = "CloseButton"; closeButton.Parent = header; closeButton.Size = UDim2.new(0.1, 0, 0.5, 0)
    closeButton.Position = UDim2.new(0.95, 0, 0.25, 0); closeButton.AnchorPoint = Vector2.new(0.5, 0.5)
    closeButton.BackgroundColor3 = Color3.fromRGB(220, 70, 70); closeButton.Text = "X"
    closeButton.TextColor3 = Color3.fromRGB(255, 255, 255); closeButton.Font = Enum.Font.GothamBold
    closeButton.TextScaled = true; closeButton.BorderSizePixel = 0
    local closeCorner = Instance.new("UICorner"); closeCorner.CornerRadius = UDim.new(0, 12); closeCorner.Parent = closeButton
    local scrollFrame = Instance.new("ScrollingFrame")
    scrollFrame.Name = "ScrollFrame"; scrollFrame.Parent = mainFrame; scrollFrame.Size = UDim2.new(0.95, 0, 0.75, 0)
    scrollFrame.Position = UDim2.new(0.025, 0, 0.22, 0); scrollFrame.BackgroundTransparency = 1; scrollFrame.BorderSizePixel = 0
    scrollFrame.ScrollBarThickness = 6; scrollFrame.ScrollBarImageColor3 = Color3.fromRGB(100, 100, 120)
    scrollFrame.AutomaticCanvasSize = Enum.AutomaticSize.Y; scrollFrame.ScrollingDirection = Enum.ScrollingDirection.Y
    local listLayout = Instance.new("UIListLayout")
    listLayout.Parent = scrollFrame; listLayout.SortOrder = Enum.SortOrder.LayoutOrder; listLayout.Padding = UDim.new(0, 5)
    local function updateCountdown()
        if not countdownLabel or not countdownLabel.Parent then return false end
        local currentTime = os.time()
    ...
    local countdownConnection = RunService.Heartbeat:Connect(function()
        if not updateCountdown() then countdownConnection:Disconnect() end
    end)
    ...
    if weeklyData.playerTimes then
        for userId, data in pairs(weeklyData.playerTimes) do
            ...
            table.insert(sortedPlayers, { userId = userId, time = currentTime, name = data.name, rank = data.rank })
        end
    end
    -- sort
    table.sort(sortedPlayers, function(a, b) return a.time > b.time end)
    if #sortedPlayers == 0 then
        ...
        noDataLabel.Name = "NoDataLabel"; noDataLabel.Parent = scrollFrame; noDataLabel.Size = UDim2.new(1, 0, 0, 40)
        noDataLabel.BackgroundTransparency = 1; noDataLabel.Text = "No time records yet for this week."
        noDataLabel.TextColor3 = Color3.fromRGB(150, 150, 150); noDataLabel.Font = Enum.Font.Gotham
        noDataLabel.TextSize = 14; noDataLabel.TextWrapped = true; noDataLabel.TextScaled = true
    else
        -- entry for each player
        for i, data in ipairs(sortedPlayers) do
            local entry = Instance.new("Frame")
            entry.Name = "PlayerEntry"; entry.Size = UDim2.new(1, 0, 0, 40)
            entry.BackgroundColor3 = Color3.fromRGB(50, 50, 70); entry.BorderSizePixel = 0
            entry.Parent = scrollFrame; entry.LayoutOrder = i
            local entryCorner = Instance.new("UICorner"); entryCorner.CornerRadius = UDim.new(0, 6); entryCorner.Parent = entry
            local rankBadge = Instance.new("Frame")
            rankBadge.Name = "RankBadge"; rankBadge.Parent = entry; rankBadge.Size = UDim2.new(0.1, 0, 0.8, 0)
            rankBadge.Position = UDim2.new(0.02, 0, 0.1, 0); rankBadge.AnchorPoint = Vector2.new(0, 0)
            if i == 1 then rankBadge.BackgroundColor3 = Color3.fromRGB(255, 215, 0)
            elseif i == 2 then rankBadge.BackgroundColor3 = Color3.fromRGB(192, 192, 192)
            elseif i == 3 then rankBadge.BackgroundColor3 = Color3.fromRGB(205, 127, 50)
            else rankBadge.BackgroundColor3 = Color3.fromRGB(60, 140, 200) end
            rankBadge.BorderSizePixel = 0
            local badgeCorner = Instance.new("UICorner"); badgeCorner.CornerRadius = UDim.new(0, 15); badgeCorner.Parent = rankBadge
            local rankText = Instance.new("TextLabel")
            rankText.Name = "RankText"; rankText.Parent = rankBadge; rankText.Size = UDim2.new(1, 0, 1, 0)
            rankText.BackgroundTransparency = 1; rankText.Text = tostring(i)
            rankText.TextColor3 = Color3.fromRGB(255, 255, 255); rankText.Font = Enum.Font.GothamBold
            rankText.TextSize = 14; rankText.TextScaled = true
            local nameLabel = Instance.new("TextLabel")
            nameLabel.Name = "NameLabel"; nameLabel.Parent = entry; nameLabel.Size = UDim2.new(0.4, 0, 0.4, 0)
            nameLabel.Position = UDim2.new(0.15, 0, 0.1, 0); nameLabel.BackgroundTransparency = 1
            nameLabel.Text = data.name; nameLabel.TextColor3 = Color3.fromRGB(255, 255, 255)
            nameLabel.TextXAlignment = Enum.TextXAlignment.Left; nameLabel.Font = Enum.Font.Gotham
            nameLabel.TextSize = 14; nameLabel.TextTruncate = Enum.TextTruncate.AtEnd; nameLabel.TextScaled = true
            local rankLabel = Instance.new("TextLabel")
            rankLabel.Name = "RankLabel"; rankLabel.Parent = entry; rankLabel.Size = UDim2.new(0.4, 0, 0.3, 0)
            rankLabel.Position = UDim2.new(0.15, 0, 0.5, 0); rankLabel.BackgroundTransparency = 1
            rankLabel.Text = "Rank: " .. data.rank; rankLabel.TextColor3 = Color3.fromRGB(180, 180, 200)
            rankLabel.TextXAlignment = Enum.TextXAlignment.Left; rankLabel.Font = Enum.Font.Gotham
            rankLabel.TextSize = 11; rankLabel.TextTruncate = Enum.TextTruncate.AtEnd; rankLabel.TextScaled = true
            local timeLabel = Instance.new("TextLabel")
            timeLabel.Name = "TimeLabel"; timeLabel.Parent = entry; timeLabel.Size = UDim2.new(0.4, 0, 0.4, 0)
            timeLabel.Position = UDim2.new(0.55, 0, 0.3, 0); timeLabel.AnchorPoint = Vector2.new(0, 0.5)
            timeLabel.BackgroundTransparency = 1; timeLabel.Text = formatTime(data.time)
            timeLabel.TextColor3 = Color3.fromRGB(100, 220, 100); timeLabel.TextXAlignment = Enum.TextXAlignment.Left
            timeLabel.Font = Enum.Font.GothamBold; timeLabel.TextSize = 14; timeLabel.TextScaled = true
        end
    end
...
local function onChatted(player, message)
    if message:sub(1, #COMMAND_PREFIX) == COMMAND_PREFIX then
        ...
        if command == "access" then
            local userId = player.UserId; local currentTime = os.time()
            if commandCooldowns[userId] and currentTime - commandCooldowns[userId] < COMMAND_COOLDOWN then
                ...
                task.delay(3, function() if privateMessage and privateMessage.Parent then privateMessage:Destroy() end end)
                return
            end
            commandCooldowns[userId] = currentTime
            -- rank requirement
            if hasGuardRank(player.UserId) then showTimeRecords(player)
            else
                local privateMessage = Instance.new("Hint")
                privateMessage.Parent = player.PlayerGui
                privateMessage.Text = "You need to be at least Guard rank to use this command."
                task.delay(5, function() if privateMessage and privateMessage.Parent then privateMessage:Destroy() end end)
            end
            ...
            if hasGuardRank(player.UserId) then
                ...
                task.delay(2, function()
                    if privateMessage and privateMessage.Parent then
                        if success then privateMessage.Text = "Test report sent to Discord!" else privateMessage.Text = " Failed to send test report." end
                    end
                end)
                task.delay(5, function() if privateMessage and privateMessage.Parent then privateMessage:Destroy() end end)
            else
...
local function trackPlayerTime(player)
    if not hasGuardRank(player.UserId) then return end
    local userId = player.UserId; local playerName = player.Name; local rankName = getPlayerRankName(userId)
    if not weeklyData.playerTimes then weeklyData.playerTimes = {} end
    if weeklyData.playerTimes[userId] then
        ...
        weeklyData.playerTimes[userId] = { name = playerName, time = 0, rank = rankName }
    end
    activeSessions[userId] = os.time(); dirtyPlayers[userId] = true
    ...
    leaveConnection = player.AncestryChanged:Connect(function(_, parent)
        if parent == nil then
            if activeSessions[userId] then
                ...
                activeSessions[userId] = nil; dirtyPlayers[userId] = true
                debouncedSave()
...
game:BindToClose(function()
    ...
    while not saveCompleted and os.time() - startWait < 5 do task.wait(0.1) end
    if saveCompleted then print("saved successfully before shutdown") else print("Save may not have completed before shutdown") end
    task.wait(1)
end)
...
while true do
    ...
    if #Players:GetPlayers() > 0 then
        ...
        for userId, startTime in pairs(activeSessions) do
            if weeklyData.playerTimes and weeklyData.playerTimes[userId] then
                local currentTime = os.time(); local sessionTime = currentTime - startTime
                weeklyData.playerTimes[userId].time = weeklyData.playerTimes[userId].time + sessionTime
                activeSessions[userId] = currentTime; dirtyPlayers[userId] = true
            end
        end
        -- auto save
        saveWeeklyData()
        -- for upcoming reset
        local currentTime = os.time(); local timeUntilReset = weeklyData.endTime - currentTime
        if timeUntilReset > 0 and timeUntilReset < 3600 then saveWeeklyData() end
        if currentTime > weeklyData.endTime then
            ...
            if finalSaveSuccess then
                if not RunService:IsStudio() then sendWeeklyReportToDiscord()
                else
                    local totalPlayers = 0; local totalTime = 0
                    for userId, data in pairs(weeklyData.playerTimes) do
                        if data and data.name then
                            totalPlayers = totalPlayers + 1; totalTime = totalTime + (data.time or 0)
            ...
            else print("WARNING: Final save failed, report may be incomplete!") end
            -- store old week summary
            local oldWeekData = { startTime = weeklyData.startTime, endTime = weeklyData.endTime, playerCount = 0, totalTime = 0 }
            if weeklyData.playerTimes then
                for userId, data in pairs(weeklyData.playerTimes) do
                    if data and data.name then
            ...
            weeklyData.startTime, weeklyData.endTime = calculateWeekBoundaries()
            weeklyData.playerTimes = {}; activeSessions = {}; dirtyPlayers = {}
            print(string.format("Weekly reset complete. Old week had %d players with total time %s", oldWeekData.playerCount, formatTime(oldWeekData.totalTime)))
            print("New week period: " .. formatDate(weeklyData.startTime) .. " to " .. formatDate(weeklyData.endTime))
            task.spawn(function() task.wait(30); saveWeeklyData() end)
        end
    end
end
